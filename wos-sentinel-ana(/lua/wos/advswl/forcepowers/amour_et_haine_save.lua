...
wOS.ForcePowers:RegisterNewPower({
		...
		action = function( self )
			...
			ply:EmitSound( swordamour1, 75, 130, 1, CHAN_AUTO )

            timer.Simple( 0.1 , function() ParticleEffect("[27]_love_slash" , ply:GetPos() + ply:GetForward() * 60 , ply:EyeAngles() + Angle( 0 , -180 , 45 ) , ply ) end )
...
wOS.ForcePowers:RegisterNewPower({
		name = "Mouvement 2 - Amour / Haine",
		icon = "",
		image = "materials/wos/skills/amour2.png",
		cooldown = 200,
		manualaim = false,
		description = "",
		help = "",
		action = function( self )
			...
			timer.Simple( 0.1 , function() ParticleEffect("[27]_love_slash_impact" , ply:GetPos() + ply:GetForward() * 60 , ply:EyeAngles() + Angle( 0 , -180 , 45 ) , ply ) end )
            timer.Simple( 0.1 , function() ParticleEffect("[27]_love_chaos_glow" , ply:GetPos() , ply:EyeAngles() + Angle(0,0,0), ply ) end )
            timer.Simple( 0.1 , function() ParticleEffect("[27]_love_dash_add_4" , ply:GetPos() , ply:EyeAngles() + Angle( 0 , 90 , 0 ) , ply ) end )
            ...
            if not IsValid( self ) then return end	
                ...
                for k , v in pairs( ents.FindInSphere( ply:GetPos() + ply:GetAimVector() * 130 , 130 ) ) do
                    ...
                    if ( IsValid( v ) and v:IsPlayer() and v != ply ) then
                        ...
                        v:TakeDamage( self.SaberDamage * 1.3 , ply , ply )
                        timer.Simple( 0.1 , function() ParticleEffect("[*]_love_slashes" , v:GetPos() , ply:EyeAngles() + Angle( 0 , 90 , 0 ) , v ) end )
                        v:EmitSound( cutjoueur, 75, math.random(50, 150), 0.8, CHAN_AUTO )
                        util.ScreenShake( ply:GetPos() , 3 , 50 , 0.5 , 300 ) 
...
wOS.ForcePowers:RegisterNewPower({
		name = "Mouvement 3 - Amour / Haine",
		icon = "",
		image = "materials/wos/skills/amour3.png",
		cooldown = 240,
		manualaim = false,
		description = "",
		help = "",
		action = function( self )
            ...
            timer.Simple( 0.1 , function() ParticleEffectAttach("[27]_love_dash" , 4 , ply , 0 ) end )
            ...
            timer.Simple( 0.4 , function() if not self:IsValid() then return end
                ...
                timer.Create( "mouvement-amour3" .. ply:SteamID64() , 0.15 , 5 , function() if not self:IsValid() then return end
...
wOS.ForcePowers:RegisterNewPower({
		name = "Mouvement 4 - Amour / Haine",
		icon = "",
		image = "materials/wos/skills/amour4.png",
		cooldown = 300,
		manualaim = false,
		description = "",
		help = "",
		action = function( self )
            ...
            timer.Simple( 0.1 , function() ParticleEffectAttach( "[27]_love_chaos" , 4 , ply , 0 ) end )
            timer.Simple( 0.8 , function() ply:StopParticles() end )
            
            timer.Create( "mouvement-amour4" .. ply:SteamID64() , 0.05 , 6 , function() if not self:IsValid() then return end
                ...
                for k , v in pairs( ents.FindInSphere(ply:GetPos() , 200 ) ) do
                    ...
                    if v:IsValid() and v != ply then
                        ...
                        if v:IsNPC() or v:IsPlayer() then
                            ...
                            dmg_mouvement4_amour:SetDamage( self.SaberDamage * 1.3 )
                            dmg_mouvement4_amour:SetDamageType( 2 )
                            dmg_mouvement4_amour:SetAttacker( ply )
                            dmg_mouvement4_amour:SetInflictor( self )
