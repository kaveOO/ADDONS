...
function rb655_RenderBlade_wos( pos, dir, len, maxlen, width, color, black_inner, inner_color, eid, underwater, quillon, bladeNum, settings, dual )
	if not pos or not dir or not len or not maxlen or not width or not color or not eid then return end
	...
	if wOS.ShouldDrawLightsaberLight:GetBool() then
		if ( !quillon ) then
			local light_id = eid + 1000 * bladeNum
			if dual then light_id = light_id + 1000 * bladeNum end
			local SaberLight = DynamicLight( light_id )
			if ( SaberLight ) then
...
function rb655_CalculateBladeLength_wos( length, wep, blade )
	if not IsValid( wep ) then return length end
	local maxlen = wep:GetUseLength( blade )
	if not maxlen then return length end
	return math.min( length, maxlen )
end

function rb655_CalculateBladeLength_wos_dual( length, wep, blade )
	if not IsValid( wep ) then return length end
	if not wep:IsWeapon() then return length end
	local maxlen = wep:GetUseSecLength( blade )
	if not maxlen then return length end
	return math.min( length, maxlen )
end
...
function rb655_ProcessLightsaberEntity_wos( ent )
	...
	for id, t in pairs( ent:GetAttachments() ) do
		...
		if ( bladeNum && ent:LookupAttachment( "blade" .. bladeNum ) > 0 ) then
			...
			rb655_ProcessBlade_wos( ent:EntIndex(), pos, ang, rb655_CalculateBladeLength_wos( ent:GetBladeLength(), ent, blades ), blades )
			bladesFound = true
		end
	...
	if ( !bladesFound ) then
		...
		rb655_ProcessBlade_wos( ent:EntIndex(), pos, ang, rb655_CalculateBladeLength_wos( ent:GetBladeLength(), ent ), 1 )
	end
...
function rb655_ProcessLightsaberEntity_dual_wos( ent )
	...
	for id, t in pairs( ent.LeftHilt:GetAttachments() ) do
		...
		if ( bladeNum && ent.LeftHilt:LookupAttachment( "blade" .. bladeNum ) > 0 ) then
			...
			rb655_ProcessBlade_wos( ent:EntIndex(), pos, ang, rb655_CalculateBladeLength_wos_dual( ent:GetSecBladeLength(), ent, blades ), blades, true )
			bladesFound = true
		end
	...
	if ( !bladesFound ) then
		...
		rb655_ProcessBlade_wos( ent:EntIndex(), pos, ang, rb655_CalculateBladeLength_wos_dual( ent:GetSecBladeLength(), ent ), 1, true )
	end
