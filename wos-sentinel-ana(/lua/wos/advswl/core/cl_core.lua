...
function wOS.ALCS:OpenForceMenu()
	...
	for key, data in pairs( powers ) do
		...
		button.Paint = function( pan, ww, hh )
			...
			if button.HelpToggle and button.Data.help then
				draw.SimpleText( button.Data.help, "wOS.DescriptionFont", hh, hh*0.65, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
			else
		...
		button.DoRightClick = function( pan )
			pan.HelpToggle = !pan.HelpToggle
		end
...
function wOS.ALCS:ShouldDisableCam()
	...
	local call = hook.Run( "wOS.ALCS.ShouldDisableCam" )
