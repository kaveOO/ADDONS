...
function wOS.ALCS.Config.Skills.SkillDatabase:Initialize()
	...
	for name, _ in pairs( wOS.SkillTrees ) do
		...
		tablename = string.gsub(tablename, "[^A-Za-z0-9\\-]", "_") //Enforce chars
		TableToSkill[ tablename ] = name
		SkillToTable[ name ] = tablename
		if not file.Exists( "wos/advswl/skills/" .. tablename, "DATA" ) then file.CreateDir( "wos/advswl/skills/" .. tablename ) end
	end
...
function wOS.ALCS.Config.Skills.SkillDatabase:LoadData( ply )
	...
	for name, _ in pairs( wOS.SkillTrees ) do
		local file_name = SkillToTable[ name ]
		ply.SkillTree[ name ] = {}
		ply.EquippedSkills[ name ] = {}
		local read = file.Read( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. "_" .. charid .. ".txt", "DATA" )
		if not read then
			read = file.Read( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. ".txt", "DATA" )
			if read then
				file.Rename( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. ".txt", "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. "_" .. charid .. ".txt" )
			end
		end
		if not read then 
			local data = {}
			data = util.TableToJSON( data )
			file.Write( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. "_" .. charid .. ".txt", data )
		else
...
function wOS.ALCS.Config.Skills.SkillDatabase:SaveData( ply )
	...
	for name, _ in pairs( wOS.SkillTrees ) do
		...
		file.Write( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. "_" .. charid .. ".txt", savedata )
	end			
...
function wOS.ALCS.Config.Skills.SkillDatabase:DeleteData( ply, charid )
	...
	for name, _ in pairs( wOS.SkillTrees ) do
		local file_name = SkillToTable[ name ]
		file.Delete( "wos/advswl/skills/" .. string.lower( file_name ) .. "/" .. steam64 .. "_" .. charid .. ".txt" )
	end
