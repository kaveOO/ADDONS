...
function SWEP:PlaceItem()
	...
	ent.SpawnCategories = table.Copy( SpawnedCategories )
...
function SWEP:SecondaryAttack()
	if CLIENT then
		...
		for i=1, 9 do
			...
			l.ID = i
			l:SetChecked( SpawnedCategories[ l.ID ] )
			l.OnChange = function( pan, val )
				if val then
					SpawnedCategories[pan.ID] = true
				else
					SpawnedCategories[pan.ID] = false
				end
			end
