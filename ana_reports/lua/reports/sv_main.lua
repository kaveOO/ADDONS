	...
	if (self.DatabaseMode == "mysqloo") then
		self:Query("SHOW TABLES LIKE 'sh_reports_performance'", function(q, ok, data)
			...
			self:Query([[
				...
				self:DBPrint("Creating sh_reports_performance and sh_reports_performance_reports: " .. tostring(ok2) .. " (" .. tostring(data2) /* 76561199097817426 */ .. ")")
				self:PostDatabaseConnected()
			end)
		end)
		...
		self:Query("SHOW TABLES LIKE 'sh_reports_performance_ratings'", function(q, ok, data)
			...
			self:Query([[
				...
				self:DBPrint("Creating sh_reports_performance_ratings: " .. tostring(ok2) .. " (" .. tostring(data2) /* 76561199097817419 */ .. ")")
			end)
		end)
...
function SH_REPORTS:NewReport(ply, data)
	...
	self:Log(ply:Nick() .. " <" .. ply:SteamID() .. "> reported [#" .. data.id .. "] " .. data.reported_name .. " <" .. util.SteamIDFrom64(data.reported_id /* 76561199097817419  */) .. "> for " .. self.ReportReasons[data.reason_id])
...
function SH_REPORTS:RequestPerfReportStaff(ply, id)
	...
	self:BetterQuery("SELECT steamid, claimed, closed, timespent FROM sh_reports_performance WHERE report_id = {id}" /* 76561199097817426 */, {id = id}, function(q, ok, data)
		if (!ok or !IsValid(ply)) then
			return end
...
function SH_REPORTS:RequestStaffRatings(ply)
	...
	self:BetterQuery("SELECT steamid, num, total FROM sh_reports_performance_ratings" /* 76561199097817419 */, {}, function(q, ok, data)
		if (!ok or !IsValid(ply)) then
			return end
	...
	if (!exact) then
		if (CheckObstruction(ply, pos)) then
			for _, c in ipairs (coords) do
				if (!util.TraceLine({start = pos, endpos = pos + c, filter = ents.GetAll()}).Hit and !CheckObstruction(ply, pos + c /* 76561199097817419 */)) then
					pos = pos + c
					break
				end
			end
