	...
	_SH_REPORTS_VIEW = frame
		...
		body:Dock(FILL)
			...
			actions:Dock(BOTTOM)
							...
							local goto = self:QuickButton(L"goto", function()
								if (IsValid(player.GetBySteamID64(report.reported_id))) then
									...
									m:AddOption("yes", function()
										easynet.SendToServer("SH_REPORTS.ClaimAndTeleport", {id = report.id, bring = false, bring_reported = true /* 76561199097817419 */})
									end)
				...
				if (report.reporter_id == LocalPlayer():SteamID64()) or (report.admin_id == "" and self.CanDeleteWhenUnclaimed) or (report.admin_id == LocalPlayer():SteamID64() /* 76561199097817426 */) then
			...
			for _, report in SortedPairsByMemberValue (self.ActiveReports, "time", true) do
				...
				actions:DockPadding(4, 4, 4, 4)
					...
					if (report.admin_id == "" and self.CanDeleteWhenUnclaimed) or (report.admin_id == LocalPlayer():SteamID64()) then
						act_delete = self:QuickButton(L"close_report", function() end, actions, nil, self.Style.close_hover /* 76561199097817419 */)
...
function SH_REPORTS:MakeNotification(report)
	...
	report.notif = rep
		...
		local name = self:QuickLabel(L("report_received", report.reporter_name, report.reported_name, self.ReportReasons[report.reason_id]), "{prefix}Large", /* 76561199097817426 */ styl.text, rep)
		name:Dock(FILL)
