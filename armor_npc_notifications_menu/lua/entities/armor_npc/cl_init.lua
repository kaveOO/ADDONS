...
function ENT:Draw()
    ...
    cam.Start3D2D(pos, ang, 0.04)
        ...
        draw.RoundedBox( 0, -w*.5 -30, -h*.5, w + 60, h, Color( 52, 152, 219, 150 ))
...
local function OpenNPCArmorVendor()
    
    local sw, sh = ScrW(), ScrH()
    local p = LocalPlayer()
    p:ConCommand( "play vo/coast/odessa/nlo_cub_hello.wav" )
    ...
    f.Paint = function(s, w, h)
        ...
        local frac = math.Clamp(p:Armor()/p:GetNWInt("PlayerMaxArmor", 100), 0, 1)
        ...
        draw.RoundedBox( 0, w*.5 - w*.12 + 1, h *.665 + 1, (w*.24 * frac - 2), h*.03 - 2, Color( 52, 152, 219, 255 ))
        draw.SimpleText( p:Armor(), "TRE28", w*.375, h *.678, Color(255, 255, 255), 2, 1)
        draw.SimpleText( p:GetNWInt("PlayerMaxArmor", 100), "TRE28", w*.625, h *.678, Color(255, 255, 255), 0, 1)
    end
    ...
    if p:IsSuperAdmin() then
        ...
        btn.DoClick = function(s)
            ...
            frame:SetSize(sw * .1, sw * .1)
            frame:SetPos( sw * .5 - sw * .05, sh * .115)
            ...
            lbl:Dock(TOP)
            lbl:SetText("Default Cost Per 1 AP?")
            lbl:SetFont("TRE20")
            ...
            local lble = vgui.Create("DLabel", frame)
            lble:Dock(TOP)
            lble:SetText("Default Max Armor?")
            lble:SetFont("TRE20")
            lble:SetContentAlignment(8)
            lble:SizeToContents()

            local tee = vgui.Create("DTextEntry", frame)
            tee:Dock(TOP)
            tee:SetValue(p:GetNWInt("PlayerMaxArmor", 100))

            local dbtn = vgui.Create("DButton", frame)
            dbtn:Dock(BOTTOM)
            dbtn:SetText("Confirm Changes")
            dbtn:SetFont("TRE20")
            dbtn.DoClick = function(s)
                net.Start("SaveArmorConfigChanges")
                    net.WriteFloat(te:GetValue())
                    net.WriteInt(tee:GetValue(), 16)
                net.SendToServer()
            end
        end
    end
    ...
    h.Paint = function(s, w, h) 
        ...
        draw.SimpleText( "If you require armor please select an option below!", "TRE20", w*.22 , h *.42 + add, Color(255, 255, 255), 0, 1 )
        if p:IsSuperAdmin() then
            draw.RoundedBox( 0, x, h*.55, w - x - w*.008, h*.4, Color( 52, 152, 219, 155 ) )
    ...
    mdl:Dock(LEFT)
    mdl:SetWide(sh * .11)
    mdl:SetModel( "models/Humans/Group03/male_03.mdl" )
    ...
    b.Paint = function(s, w, h)
        draw.RoundedBox(8, 0, 0, w, h, Color( 52, 152, 219, 255 ))
    end
    ...
    for k,v in SortedPairs(ARMOR_VALUES_MULTIPLIERS) do
        ...
        hp.Paint = function(s,w,h)
            ...
            local armor = p:Armor()
            local maxarmor = p:GetNWInt( "PlayerMaxArmor", 100 )
            local quantity = math.Round(maxarmor * v)
            local IsMax = (quantity == maxarmor) and true or false
            quantity = IsMax and "MAX" or quantity
            local price = math.Round((e:GetNWInt("CostPerArmor") * maxarmor) * v)
            
            local z = sh * .075

            col = s:IsHovered() and Color( 32, 132, 199 ) or Color( 52, 152, 219 )
            ...
            draw.SimpleText( "$"..price, "TRE35", w*.85, h*.5, Color( 52, 152, 219, 255 ), 2, 1 ) 
        end
    end
    f:ShowCloseButton( false )
end

net.Receive("OpenArmorNPCMenu",function()
    OpenNPCArmorVendor()
end)